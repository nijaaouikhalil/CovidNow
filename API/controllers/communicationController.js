...
exports.getAllMessages = (req, res) => {
  ...
  communication.find({doctorId: doctorId, userId: userId}, "sendersId message date emergency").sort({date: 1}).exec((err, messages) => {
    if (err) {
      res.status(500).send({ message: err });
      return;
    }
...
exports.getImportantMessage = (req, res) => {
  ...
  communication.find({doctorId: doctorId, userId: userId, emergency: true}, "sendersId message date emergency").sort({date: 1}).exec((err, messages) => {
    if (err) {
      res.status(500).send({ message: err });
      return;
    }

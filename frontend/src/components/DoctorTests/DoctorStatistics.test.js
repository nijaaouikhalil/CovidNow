...
describe("Doctor Statistics Page", () => {
  ...
  it("Displays zero when no users passed", () => {
    ...
    const countAppointments = screen.getByTestId(
      "doctor-stats-count-appointments"
    );
    const countCovidCases = screen.getByTestId(
      "doctor-stats-count-covid-cases"
    );
  ...
  it("Displays correct counts if users passed", async () => {
    ...
    render(
      <Provider store={store}>
        <MemoryRouter>
          <DoctorStatistics patients={all_users} />
        </MemoryRouter>
      </Provider>
    );
    const countCases = screen.getByTestId("doctor-stats-count-cases");
